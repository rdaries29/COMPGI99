...
class Worker(object):
    ...
    def __init__(self,name,env,thread_index,policy_net,value_net,global_counter,discount_factor=0.99,summary_writer=None,max_global_steps=None):
        ...
        # Check out defintions for line of code self.sp = StateProcessor()

        self.summary_writer = summary_writer

        with tf.variable_scope(self.name):

            self.local_policy_net = Policy_Network()
            self.local_value_net = Value_Network()

        self.copy_params_op = make_copy_params_op(
            tf.contrib.slim.get_variables(scope="global", collection=tf.GraphKeys.TRAINABLE_VARIABLES),
            tf.contrib.slim.get_variables(scope=self.name, collection=tf.GraphKeys.TRAINABLE_VARIABLES))

        self.vnet_train_op = make_train_op(self.local_value_net, self.global_value_net)
        self.pnet_train_op = make_train_op(self.local_policy_net, self.global_policy_net)
        ...
        def run(self, sess, coord, t_max):
            with sess.as_default(), sess.graph.as_default():
                ...
                # Create preprocessing function to assist with preprocessing for initial states
                # Initial state
        ...
        def run_n_steps(self, n, sess):
            ...
            for _ in range(n):
                ...
                # Call preprocessing function to handle next state
                ...
                if done:
                    # Check situation for end of sequence
                    self.state = atari_helpers.atari_make_initial_state(self.sp.process(self.env.reset()))
                    break
                else:
